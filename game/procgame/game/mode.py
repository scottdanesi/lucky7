...
class Mode(object):
    ...
    def __scan_switch_handlers(self):
        ...
        handler_func_re = re.compile(
            'sw_(?P<name>[a-zA-Z0-9]+)_(?P<state>open|closed|active|inactive)(?P<after>_for_(?P<time>[0-9]+)(?P<units>ms|s))?')
        for item in dir(self):
            ...
            if switch_name not in self.game.switches:
                raise ValueError('Unrecognized switch name %s in handler %s.%s().' % (
                    switch_name, self.__class__.__name__, item))
    ...
    def add_switch_handler(self, name, event_type, delay, handler):
        ...
        try:
            ...
            self.game.logger.error(
                "WARNING: add_switch_handler(): Switch %s unknown. Please check your machine configuration file." % (
                    name))
            return
        d = {'name': name, 'type': et, 'delay': delay, 'handler': handler, 'param': sw}
        if d not in self.__accepted_switches:
            self.__accepted_switches.append(
                Mode.AcceptedSwitch(name=name, event_type=et, delay=delay, handler=handler, param=sw))
    ...
    def handle_event(self, event):
        ...
        for accepted in filter(filt, self.__accepted_switches):
            if accepted.delay is None or accepted.delay == 0:
                handler = accepted.handler
                result = handler(self.game.switches[accepted.name])
                if result == SwitchStop:
                    handled = True
            else:
                self.delay(name=sw_name, event_type=accepted.event_type, delay=accepted.delay, handler=accepted.handler,
