...
class AttrCollection(object):
    ...
    def __iter__(self):
        for item in self.__items_by_number.values():
            yield item
    ...
    def __contains__(self, item):
        if type(item) == str:
            return item in self.__items_by_name
        else:
            return item in self.__items_by_number
...
class Driver(GameItem):
    ...
    def patter(self, on_time=10, off_time=10, original_on_time=0, now=True):
        ...
        self.logger.debug("Driver %s - patter on:%d, off:%d, orig_on:%d, now:%s", self.name, on_time, off_time,
    ...
    def pulsed_patter(self, on_time=10, off_time=10, run_time=0, now=True):
        ...
        self.logger.debug("Driver %s - pulsed patter on:%d, off:%d, run_time:%d, now:%s", self.name, on_time, off_time,
                          run_time, now)
...
class VirtualDriver(Driver):
    ...
    def update_state(self, state):
        ...
        if not state['state']:
            ...
            self.schedule(state['timeslots'], state['outputDriveTime'], state['waitForFirstTimeSlot'])
    ...
    def pulse(self, milliseconds=None):
        ...
        self.logger.debug("Time: %f: VirtualDriver %s - pulse %d. End time: %f", time.time(), self.name, milliseconds,
                          self.time_ms)
