...
def config_named(name):
    if not os.path.isfile(name):  # If we cannot find this file easily, try searching the config_path:
        ...
        if found_path:
            ...
            return None
    return yaml.load(open(name, 'r'), Loader=yaml.FullLoader)
...
class GameController(object):
    ...
    def load_settings(self, template_filename, user_filename):
        ...
        *user_filename* contains the values set by the user.
        ...
        """
        self.user_settings = {}
        self.settings = yaml.load(open(template_filename, 'r'), Loader=yaml.FullLoader)
        if os.path.exists(user_filename):
            self.user_settings = yaml.load(open(user_filename, 'r'), Loader=yaml.FullLoader)
    ...
    def load_game_data(self, template_filename, user_filename):
        ...
        *user_filename* contains the values set by the user.
        ...
        """
        self.game_data = {}
        template = yaml.load(open(template_filename, 'r'), Loader=yaml.FullLoader)
        if os.path.exists(user_filename):
    ...
    def run_loop(self, min_seconds_per_cycle=None):
        ...
        try:
            ...
            if loops != 0:
                ...
                print("\nTotal loops: %d" % loops)
                print("Total time: %0.3fs" % dt)
                print("\nOverall loop rate: %0.3fHz\n" % float(int(loops) / int(dt)))
