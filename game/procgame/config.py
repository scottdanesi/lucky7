...
def value_for_key_path(keypath, default=None):
    ...
    for component in keypath.split('.'):
        if v is not None and component in v:
            v = v[component]
        else:
            v = default
    return v
...
def load():
    ...
    try:
        values = yaml.load(open(path, 'r'), Loader=yaml.FullLoader)
    except yaml.scanner.ScannerError as e:
        logger.error(
            'Error loading pyprocgame config from %s; your configuration file has a syntax error in it!\nDetails: %s',
            path, e)
